...
public class Circle extends ClosedShape {
	...
	protected boolean overLap(Triangle tri){
		...
		double y =getY();
		double radius = getRadius();
		if(DistanceUtil.distance(x, tri.getX1(), y, tri.getY1())>radius) return false;
		if(DistanceUtil.distance(x, tri.getX2(), y, tri.getY2())>radius) return false;
		if(DistanceUtil.distance(x, tri.getX3(), y, tri.getY3())>radius) return false;
		return true;
	}
	...
	protected boolean overLap(Donut donut){
		...
		if(this.radius>=dis+donut.getOutterRadius()){
			return true;
		}else{
			return false;
		}
	...
	protected boolean overLap(Circle circle) {
		...
		if(radius>=dis + circle.getRadius()) return true;
		return false;
